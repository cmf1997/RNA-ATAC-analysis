...
#' @param reference.genome genome of interested species. It should be custom txdb or file path in sqlite format'.
...
#'                     function(x){x[1]})[4:25]
#'
#' results <- cinaR(bed, contrasts, reference.genome = "GRCm39.sqlite")
#' }
#'
#'
...
cinaR <-
  function(matrix,
    ...
    if (is.null(reference.genome)) {
      stop("'reference.genome' is not set")
    }
...
#'
#' @param cp bed formatted consensus peak matrix: CHR, START, STOP and raw peak counts (peaks by 3+samples)
#' @param reference.genome genome of interested species. It should be custom txdb or file path
#' @param show.annotation.pie shows the annotation pie chart produced with ChipSeeker
#' @param verbose prints messages through running the pipeline
#'
...
annotatePeaks <-
  function(cp,
    ...
    if (file.exists(reference.genome)) {
      txdb <- AnnotationDbi::loadDb(reference.genome)
    } else {
      stop("reference.genome should be custom txdb or file path in sqlite format")
    }

    # annotate peaks define tssRegion
    seqlevelsStyle(bed.GRanges) <- "NCBI"
    annoPeaks <- ChIPseeker::annotatePeak(bed.GRanges, TxDb = txdb, tssRegion=c(-3000, 3000),verbose = verbose)
  ...
  if (DA.choice == 1) {
    ...
    for (i in seq_len(ncol(ccc))) {
      ...
      if (experiment.type == "ATAC-Seq"){
        ...
        top.table <- res.significant[, c(1:15, 18, 22)]
        colnames(top.table)[c(16, 17)] <- c("logFC", "FDR")
      } else {
